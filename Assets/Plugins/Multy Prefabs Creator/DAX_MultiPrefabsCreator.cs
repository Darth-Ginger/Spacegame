	...
	bool DAXcreatePrefab( string location, object gObject)
	{
		try
		{
			...
			{*/
				...
				if (EXEnabled && overwritePosition) {GO.transform.localPosition = positionBaseVector; };
				if (EXEnabled && overwriteRotation) {GO.transform.localRotation = Quaternion.Euler( rotationBaseVector ); };
				if (EXEnabled && overwriteScale) {GO.transform.localScale = scaleBaseVector; };

                if (replaceByName && System.IO.File.Exists( location ))
                {
                    Object Obj = AssetDatabase.LoadAssetAtPath(location, typeof(Object));
#if UNITY_2018_3_OR_NEWER
                    PrefabUtility.SaveAsPrefabAsset(GO, location);
#else
                ...
                {
#if UNITY_2018_3_OR_NEWER
                    PrefabUtility.SaveAsPrefabAssetAndConnect(GO, location, InteractionMode.AutomatedAction);
#else
                ...
                if (EXEnabled && overwriteScale) {GO.transform.localScale = scale; };
				if (EXEnabled && overwriteRotation) {GO.transform.localRotation = rotation; };
				if (EXEnabled && overwritePosition) {GO.transform.localPosition = trans; };
			/*}
			else
			{                
                if (replaceByName && System.IO.File.Exists(location))
